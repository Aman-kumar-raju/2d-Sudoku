import pygame
pygame.font.init()
Window = pygame.display.set_mode((505, 600))
pygame.display.set_caption("SUDOKU GAME by AMAN & team.")
x = 0
z = 0
diff = 500 / 9
value= 0
...
font1 = pygame.font.SysFont("comicsans", 17)
def cord(pos):
    global x
    x = pos[0]//diff
    global z
...
def highlightbox():
    for k in range(2):
        pygame.draw.line(Window, (255, 0, 0), (x * diff-3, (z + k)*diff), (x * diff + diff + 3, (z + k)*diff), 7)
        pygame.draw.line(Window, (255, 0, 0), ( (x + k)* diff, z * diff ), ((x + k) * diff, z * diff + diff), 7)  
       
def drawlines():
    for i in range (9):
        for j in range (9):
            if defaultgrid[i][j]!= 0:
                pygame.draw.rect(Window, (0, 153, 153), (i * diff, j * diff, diff + 1, diff + 1))
...
def gameresult():
    text1 = font.render("game finished Press R or D ", 1, (0, 0, 0))
    Window.blit(text1, (20, 550)) 
flag=True  
flag1 = 0
flag2 = 0
